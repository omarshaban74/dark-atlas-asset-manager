...
class AssetCreate(BaseModel):
    ...
    first_seen: Optional[datetime] = Field(default=None, description="When the asset was first recorded")
    last_seen: Optional[datetime] = Field(default=None, description="When the asset was last sighted")
    source: str = Field(description="One of: import, scan, manual")
    tags: List[str] = Field(default=[], description="Free-form labels")
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="Type-specific fields")
    asset_metadata: Optional[Dict[str, Any]] = Field(default=None, description="Type-specific fields")

    def get_metadata(self) -> dict:
        """Returns whichever metadata field was provided, merging both if both present."""
        base = self.asset_metadata or {}
    ...
    for item in assets:
        try:
            ...
            if existing:
                ...
                existing_meta.update(item.get_metadata())
                ...
                db.add(Asset(
                    ...
                    value=item.value,
                    status=item.status,
                    first_seen=item.first_seen.isoformat() if item.first_seen else now,
                    last_seen=item.last_seen.isoformat() if item.last_seen else now,
                    source=item.source,
                    tags=json.dumps(item.tags),
                    asset_metadata=json.dumps(item.get_metadata()),
                    risk_score=None,
                    environment=None,
                    criticality=None,
                    ai_summary=None,
